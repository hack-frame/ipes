...
int find_sock_type(struct Unit_Block * unit)
{
    ...
    ret = 0;
    if ((value = get_value_by_option(unit, "socket_type")) == NULL)
        value = create_str("SOCK_STREAM");
printf("-> %s\n", value);
    if (strcmp(value, "SOCK_STREAM") == 0)
        ret = 1;
    else
        fprintf(stderr, "!!!!! sock type is not full !!!!!");
...
int find_family_type(struct Unit_Block * unit)
{
    ...
    ret = 0;
    if ((value = get_value_by_option(unit, "conection_type")) == NULL)
        value = create_str("AF_INET");

    if (strcmp("AF_INET", value) == 0)
        ret = 2;
    else
